...
module.exports = {
    signup : async (req, res) => {
        ...
        try {
            //3. 존재하는 이메일인지 확인하기. 이미 존재하는 이메일이면 ALREADY ID 반환
            ...
            //5. 2차 세미나 때 배웠던 pbkdf2 방식으로 (비밀번호 + salt) => 암호화된 password
    ...
    signin : async (req, res) => {
        async (req, res) => {
            ...
            try {
                ...
                //4. 비밀번호 확인하기 - 로그인할 email의 salt를 DB에서 가져와서 사용자가 request로 보낸 password와 암호화를 한 후 DB에 저장되어있는 password와 일치하면 true
                ...
                return res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.SIGN_UP_SUCCESS, { id, email, userName }));
